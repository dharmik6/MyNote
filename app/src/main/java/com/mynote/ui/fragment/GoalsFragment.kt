...
import com.mynote.databinding.FragmentGoalsBinding
...
import com.mynote.ui.adapter.GoalAdapter
...
@AndroidEntryPoint
class GoalsFragment : BaseFragment(), OnClickListener, OnLongClickListener, OnColorSelectListener {

    @Inject
    lateinit var prefUtils: PrefUtils
    private val mViewModel: FirebaseUserViewModel by viewModels()
    private val binding by viewBinding { FragmentGoalsBinding.inflate(layoutInflater) }
    ...
    override fun onCreateView(
        ...
        (activity as HomeActivity).binding.layout.setOnCheckedChangeListener{ btn ,ischecked ->
            ...
            if (ischecked){
                binding.rvGoal.layoutManager = StaggeredGridLayoutManager(2,
                    StaggeredGridLayoutManager.VERTICAL)
            }else{
                binding.rvGoal.layoutManager = LinearLayoutManager(requireContext())
            }
            setRecyclerView()
            getData()
        }
    ...
    private fun setRecyclerView() {
        ...
        adapter = GoalAdapter(requireContext(), mutableListOf(),this)
